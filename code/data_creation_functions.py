...
def run_model(params):
    ...
    with open("params/{}_measures.json".format(mode), "r", encoding="utf-8") as fp:
        measures = json.load(fp)
    with open(
        "params/{}_simulation_parameters.json".format(mode), "r", encoding="utf-8"
    ) as fp:
    ...
    simulation_params["transmission_risk_vaccination_modifier"] = {
        "reception": vaccination_modification,
        "transmission": 0,
    }
    ...
    G = nx.readwrite.gpickle.read_gpickle(
        join(
            contact_network_src,
            "{}_fraction-{}_{}.bz2".format(
                fname, presence_fraction, contact_network_type
            ),
        )
    )
    ...
    model = SEIRX_uni(
        ...
        transmission_risk_ventilation_modifier=measures[
        ...
        if (
            len(
                [
                    ...
                    if (a.exposed is True or a.infectious is True)
                ]
            )
            == 0
        ):
    ...
    row["unvaccinated_lecturers"] = len(
        [
            a
            for a in model.schedule.agents
            if a.type == "lecturer" and a.vaccinated == False
        ]
    )
    # vaccinated lecturers
    row["vaccinated_lecturers"] = len(
        [
            a
            for a in model.schedule.agents
            if a.type == "lecturer" and a.vaccinated == True
        ]
    )
    # infected unvaccinated lecturers
    row["infected_unvaccinated_lecturers"] = len(
        [
            a
            for a in model.schedule.agents
            if a.type == "lecturer" and a.vaccinated == False and a.recovered == True
        ]
    )
    # infected vaccinated lecturers
    row["infected_vaccinated_lecturers"] = len(
        [
            a
            for a in model.schedule.agents
            if a.type == "lecturer" and a.vaccinated == True and a.recovered == True
        ]
    )

    # unvaccinated unistudents
    row["unvaccinated_unistudents"] = len(
        [
            a
            for a in model.schedule.agents
            if a.type == "unistudents" and a.vaccinated == False
        ]
    )
    # vaccinated unistudents
    row["vaccinated_unistudents"] = len(
        [
            a
            for a in model.schedule.agents
            if a.type == "unistudents" and a.vaccinated == True
        ]
    )
    # infected unvaccinated unistudents
    row["infected_unvaccinated_unistudents"] = len(
        [
            a
            for a in model.schedule.agents
            if a.type == "unistudents" and a.vaccinated == False and a.recovered == True
        ]
    )
    # infected vaccinated unistudents
    row["infected_vaccinated_unistudents"] = len(
        [
            a
            for a in model.schedule.agents
            if a.type == "unistudents" and a.vaccinated == True and a.recovered == True
        ]
    )
...
def run_ensemble(
    ...
    u_mask=False,
    l_mask=False,
    u_vaccination_ratio=0.0,
    l_vaccination_ratio=0.0,
    presence_fraction=1.0,
    u_screen_interval=None,
    l_screen_interval=None,
    testing=False,
    ...
    bmap = {True: "T", False: "F"}
    measure_string = (
        "university_lmask-{}_umask-{}_pfrac-{}".format(
            bmap[l_mask], bmap[u_mask], presence_fraction
        )
    ...
    ensemble_results.to_csv(join(res_path, measure_string + ".csv"), index=False)
